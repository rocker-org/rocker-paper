...
a container as analgous to loading an R package; it makes a set of software
functions available.  A Docker "image" is a binary archive of that software,
analgous to an R binary package -- a given version need be downloaded only
...
over a web interface. 
    ...
    sudo docker run -d -p 8787:8787 -e PASSWORD=<PICK-A-PASSWORD> rocker/rstudio
...
by pasting its IP address or DNS name (followed by the chosen port, RStudio's default `:8787` in the above example)
...
e.g. `docker run -t rocker/r-devel`.
