...
4. `testing` and `unstable` do not recieve the attention to security updates from the Debian security team \citep{debian_security}.
...
docker-friendly `init` system, s6 \citep{s6} for running persistant services,
...
Jetstream servers \citep{jetstream}, or private or institutional server hardware.  Being
...
see the Rocker website, <https://rocker-project.org>.
...
`littler` \citep{littler} scripts which provide a concise syntax for installing packages in Dockerfiles, e.g.
...
Rocker Project, such as the Dockerfile Best Practices \citep{best_practices}, the use of automated builds,
