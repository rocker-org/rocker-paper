...
a container as analogous to loading an R package; it makes a set of software
functions available.  A Docker "image" is a binary archive of that software,
analogous to an R binary package -- a given version need be downloaded only
once and then can be "run" to create a container whenever it is needed.
A "Dockerfile" is a recipe, the source-code, to create a Docker image.  Development
and contributions to the Rocker project focuses on the construction, organization
...
- **Interfaces**: Console-based (R, bash, etc) or RStudio
...
<!-- DE: It is a little weird to have this first as it is a somewhat
advanced, and I'd conjecture "proportionally speaking, "rare" use.
Isn't the main use to just "deploy an application" _locally_? I how we
hand users R, or R-devel, or RStudio, or Tibbliesverse ?

By the same token, it is a bit weird to intro Singularity _here_ and I
still don't know if Shifter really needs to be in the paper (cf
Patrick's comments)

So should we maybe insert a really short para like this

   Deployment: Ready-to-run binaries
   
 

nd then start the para with 'Another' instead on one?  There is a
little overlap with 'Sandboxed' below but I think it is worth it to
start with "first things first" here

-->

Docker permits convenient access to pre-configured and create
_binary_ images that "just work."  This allows R users to access a
wider-variety of ready-to-use environments than the R Project
itself provides.  For example, R users on Windows may run RStudio
Server or Shiny Server locally just by launching a single command
(once Docker itself is installed), access R-devel without affecting
their local system and more.  Here, we detail the some of the principle
use cases motivate these containerized versions of R environments
and the design principles that help make them work. 
...
by first installing the appropriate Docker command (or distribution) for Windows, MacOS, and Linux-based
...
The `docker run` option `-p` sets the port on which RStudio will appear, 8787 is
the RStudio default (adding your user to the docker group to avoids the
nedd for a `sudo` command to call `docker`: `sudo usermod -g docker $USER`). 
...
by pasting its IP address or DNS name (followed by the chosen port, e.g. `:8787`)
into a browser and entering the appropriate password.  
This provides the user with a familiar,
...
runtime enviornment requires.  To work around this problem, Lawrence Berkeley National
Labs (LBNL) has made `Singularity`: a container runtime environment that users can
both install and use to run most Docker containers without requiring root privileges\citep{singularity},
...
Users can access a root `bash` shell in a running Rocker container using 
...
<!-- DE False. On Linux the dockerd has the correct rights, and I (as member of group Docker) 
can access docker without it.  That is important: minimal sudo use!  -->
...
them sandboxed from the host container.  Further, Docker images can
be deployed from a machine without root privileges through the alternate container
runtime environment, Singularity \citep{singularity}.  Unlike traditional virtual machines,
...
Having these public source files built automatically by a trusted provider (Docker Hub),
rather than built locally and uploaded only in binary form provides is also useful from a
security perspective in avoiding malware. 
...
image, e.g. `rocker/verse:3.4.0`.  If no tag is explicitly requested,
...
<!--
however, `packrat` should not be necessary
...
(e.g. `debian:jessie`, `debian:stretch`)
...
Docker commands without an organization namespace, e.g. `docker run -t
...
e.g. `docker run -ti rocker/r-devel`.
...
[^1]: Includes 49,000 under eariler name, `hadleyverse`.
...
the corresponding version tag, e.g. `rocker/r-ver:3.4.2` at the time
...
version must be compiled from source (e.g. the Gibbs Sampling program
