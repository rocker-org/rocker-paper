...
and run Docker containers without root privileges, as most cluster administrators require [@singularity].  
Another LBNL project, *Shifter*, allows Docker containers to be deployed on leadership-scale
high performance computing platforms [@shifter].


Importantly, Rocker containers are just as easy to run on locally on most modern laptops
by first installing the appropraite Docker distribution for Windows, MacOS, and Linux-based
...
containerized environment [@Boettiger2015]. Being
...
This portablility is also valuable in an instructional context. Requiring students
...
Users building on the version-tagged images will by default use the MRAN snapshot mirror [@MRAN]
...
and thus not change versions (though it will recieve security patches).  Packages installed from BioConductor
using `bioclite()` utlitity will also install the version appropriate to the version of R found on the system
(the Bioconductor semi-annual release model avoids the need for an MRAN mirror). Users installing packages
from GitHub or other sources can request a specific git release tag or hash for a more reproducibile build,
or adopt a more comprehensive solution such as packrat [@packrat]; however, @packrat should not be necessary
for just maintaining reproducibility with respect to CRAN and Bioconductor stacks installed on versioned images.
A more general discussion of the use of Docker for computational reproducibility can be found in [@Boettiger2015]. 
...
To make extensiosn transparent and persistent, Rocker images can be extended by
...
if the RStudio interace is desired, a user might start with `FROM rocker/rstudio`;
